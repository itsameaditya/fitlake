...
def build_spark_session() -> SparkSession:
    return (
        SparkSession.builder.appName("FitLake-Silver-Transformation")
        .config(
...
def validate_bounds(
    ...
    for col_name, (lo, hi) in bounds.items():
        ...
        rejection_parts.append(
            F.when(
                ...
                F.concat(
                    F.lit(f"{col_name} out of range [{lo},{hi}]: "),
    ...
    rejection_reason = (
        F.concat_ws("; ", *rejection_parts) if rejection_parts else F.lit("")
    )

    valid_df = df.filter(is_valid)
    invalid_df = df.filter(~is_valid).select(
...
def transform_hrv(df: DataFrame) -> DataFrame:
    ...
    return (
        ...
        .filter(~F.col("is_anomaly"))  # Remove known sensor anomalies
    )
...
def transform_sleep(df: DataFrame) -> DataFrame:
    ...
    return (
        ...
        .withColumn(
            "deep_pct",
            F.round(
                F.col("deep_sleep_minutes") / F.col("total_sleep_minutes") * 100, 1
            ),
        )
        .withColumn("date", F.to_date(F.col("date")))
...
def run_silver_transformation(spark: SparkSession) -> None:
    ...
    hrv_valid, hrv_bad = validate_bounds(
        hrv_deduped, HRV_BOUNDS, "reading_id", "bronze.hrv_readings"
    )
    ...
    sleep_valid, sleep_bad = validate_bounds(
        sleep_deduped, SLEEP_BOUNDS, "record_id", "bronze.sleep_records"
    )
    ...
    all_bad = reduce(
        lambda a, b: a.unionByName(b, allowMissingColumns=True), quarantine_records
    )
    if all_bad.count() > 0:
