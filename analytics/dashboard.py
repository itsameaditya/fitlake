    ...
    if (output_dir / "daily_recovery.json").exists():
        ...
        merged = hrv_df.merge(
            sleep_df[["user_id", "date", "total_sleep_hours", "sleep_efficiency_pct",
                       "rem_sleep_minutes", "deep_sleep_minutes", "spo2_avg_pct", "resting_hr_bpm"]],
            on=["user_id", "date"],
        )
...
col1.metric("Avg Recovery", f"{avg_recovery:.0f}%",
            delta=f"{avg_recovery - 50:.0f} vs 50 baseline")
col2.metric("Green Days", f"{green_days}/{len(rec_df)}",
            delta=f"{green_days / max(len(rec_df), 1) * 100:.0f}%")
col3.metric("Avg Daily Strain", f"{avg_strain:.1f}/21")
col4.metric("Avg Sleep", f"{avg_sleep:.1f}h",
            delta=f"{avg_sleep - 8:.1f}h vs 8h ideal")
...
if len(rec_df) > 0:
    ...
    # Background zones
    fig_recovery.add_hrect(y0=0, y1=33, fillcolor="rgba(255,82,82,0.1)",
                           line_width=0, annotation_text="Red Zone")
    fig_recovery.add_hrect(y0=33, y1=66, fillcolor="rgba(255,193,7,0.1)",
                           line_width=0, annotation_text="Yellow Zone")
    fig_recovery.add_hrect(y0=66, y1=100, fillcolor="rgba(76,175,80,0.1)",
                           line_width=0, annotation_text="Green Zone")

    # Recovery line
    fig_recovery.add_trace(go.Scatter(
        x=rec_df["date"], y=rec_df["recovery_score"],
        mode="lines+markers",
        name="Recovery Score",
    ...
    if "hrv_baseline" in rec_df.columns:
        fig_recovery.add_trace(go.Scatter(
            x=rec_df["date"], y=rec_df["hrv_baseline"],
            mode="lines", name="HRV Baseline (ms)",
    ...
    fig_recovery.update_layout(
        xaxis_title="Date", yaxis_title="Recovery Score (0-100)",
...
with col_left:
    ...
    if len(strain_df) > 1 and len(rec_df) > 1:
        ...
        fig_scatter = px.scatter(
            scatter_data,
            x="strain_score", y="recovery_score",
            color="strain_category",
            trendline="ols",
            labels={"strain_score": "Day N Strain (0-21)",
                    "recovery_score": "Day N+1 Recovery (0-100)"},
            color_discrete_map={
                "Recovery": "#4CAF50", "Light": "#8BC34A",
                "Moderate": "#FFC107", "Hard": "#FF5722", "All Out": "#F44336",
            },
            height=350,
        )
...
with col_right:
    ...
    if len(rec_df) > 0:
        components = ["hrv_component", "rhr_component", "sleep_component", "spo2_component"]
        labels = ["HRV (40%)", "Resting HR (25%)", "Sleep (25%)", "SpO2 (10%)"]
        avg_components = [rec_df[c].mean() if c in rec_df.columns else 0 for c in components]
...
if len(sleep_df) > 0:
    ...
    if "sleep_debt_hours" in sleep_df.columns:
        fig_sleep.add_trace(go.Scatter(
            x=sleep_df["date"], y=sleep_df["sleep_debt_hours"],
            mode="lines", name="Sleep Debt (14d rolling)",
            line=dict(color="#FF5722", width=2, dash="dash"),
            yaxis="y2",
        ))
        fig_sleep.update_layout(
            yaxis2=dict(title="Sleep Debt (h)", overlaying="y", side="right", showgrid=False)
        )
...
if len(rec_df) >= 14:
    ...
    fig_heatmap = px.imshow(
        pivot,
        color_continuous_scale=[[0, "#F44336"], [0.33, "#FF9800"],
                                 [0.66, "#FFC107"], [1, "#4CAF50"]],
        zmin=0, zmax=100,
